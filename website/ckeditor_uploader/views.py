...
def groupReq(user):
    group_required = ["ChiefEditor", "Editor", "Reporter"]
    if user.groups.first().name in group_required:
        return True
    return False
...
class ImageUploadView(generic.View):
    ...
    def post(self, request, **kwargs):
        ...
        if request.user.groups.first().name not in group_required:
            return HttpResponse("Your IP has been logged and sent to local authorities.")
        ...
        try:
            outOfCK = escape(request.GET['outofck'])
        except:
            outOfCK = 0
        ...
        self._compress_image(backend, saved_path)
    ...
    def _save_file(request, uploaded_file):
        ...
        saved_path = default_storage.save(
            fName + '_' + get_random_string(length=10) + file_extension, uploaded_file)
    ...
    @staticmethod
    def _compress_image(backend, saved_path):
        backend.save_compressed(saved_path)
...
@user_passes_test(groupReq, login_url='/ratoAdmin/login/')
def browse(request):
    ...
    dir_list = sorted(
        set(os.path.dirname(f['src']) for f in files), reverse=False)
